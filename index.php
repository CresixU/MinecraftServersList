            ...
            function ShowPromotedServersPanel() {
                ...
                .done(res => {
                    ...
                    for(var i=0;i<5;i++) {
                        ...
                        let serverFormattedVersions = ReturnServerVersions(currentServer.minecraftServerVersions).formatedVersions;
                        if(serverFormattedVersions == null || serverFormattedVersions == undefined) serverFormattedVersions = '??'

                        let serverVersionString = ReturnServerVersions(currentServer.minecraftServerVersions).versionsString;
                        if(serverVersionString == null || serverVersionString == undefined) serverVersionString = '??'

                        $('.promoted-servers-items').append($('<div class="promoted-server-item"><div class="promoted-server-title"><a class="simple-button" href="/server.php?id='+currentServer.server.id+'">'+currentServer.server.name+'</a></div><div class="promoted-server-players"><i class="icon icon-players"></i><div class="server-players-number">'+onlinePlayers+'/'+serverSize+'</div><i class="icon '+onlineLight+'"></i></div><div class="promoted-server-rate"><i class="icon icon-rate"></i><div class="rate-number">'+currentServer.rate.rate+'</div><div class="stars" id="starsP_'+i+'"> </div> </div><div class="promoted-server-mode"><i class="icon icon-www"></i><div class="promoted-server-status">Online mode: <i class="icon '+onlineModeIcon+'"></i></div></div><div class="promoted-server-version"><i class="icon icon-version"></i><div class="version-number" title="'+serverVersionString+'">Wersja serwera: '+serverFormattedVersions+'</div></div><div class="promoted-server-web"><div class="server-url"><a href="'+currentServer.server.homepage+'">'+currentServer.server.homepage+'</a></div></div></div>'));
